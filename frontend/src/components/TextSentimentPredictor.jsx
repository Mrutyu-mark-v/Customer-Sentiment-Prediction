  ...
  <div className="max-w-4xl mx-auto">
        ...
        <div className="p-6 space-y-4">
          <div className="space-y-3">
            ...
            <p className="text-xs text-gray-600 text-center">
              Upload a CSV file containing text data for bulk prediction
            </p>
          </div>
      ...
      <AnimatePresence mode="wait">
        {error && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="bg-gray-100 border border-gray-400 rounded-xl p-4"
          >
    ...
    <div className="mt-6 text-center">
      <div className="inline-flex items-center gap-2 bg-white rounded-full px-4 py-2 shadow-sm border border-gray-300">
        <div className={`w-2 h-2 rounded-full ${loading ? 'bg-gray-500 animate-pulse' : error ? 'bg-gray-700' : 'bg-gray-800'}`} />
        <span className="text-sm text-gray-700">
          {loading ? "Connecting to model..." : error ? "Connection error" : "Ready to analyze sentiment"}
        </span>
      </div>
    </div>
  </div>
